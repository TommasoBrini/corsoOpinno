...
import exception.ApartmentAlreadyBookedException;
import exception.ApartmentNotAvailableException;
...
public class AirBnb {
     ...
     */
    private Map<UUID, Host> hostMap = new HashMap<>();
     ...
     * @param start data d'inizio prenotazione
     ...
     * @throws ApartmentNotAvailableException se l'appartamento non accetta prenotazioni per il periodo specificato
     */
    public void book(User user, Apartment apartment, LocalDate start, LocalDate end) throws ApartmentAlreadyBookedException, ApartmentNotAvailableException {
        // verifica della disponibilità dell'appartamento nel periodo indicato
        List<Apartment.Availability> availabilities = apartment.getAvailabities();
        for (Apartment.Availability availability : availabilities){
            ...
            else throw new ApartmentNotAvailableException(apartment, start, end);
        }
    ...
    private Host getMostPopularHost(LocalDateTime fromDate) {
        ...
        for (User user : getUsers()) {
            for (Booking booking : user.getBookings()) {
                // npn consideriamo le prenotazioni effettuate prima della data specificata
                if (fromDate.isBefore(booking.getTime())) continue;
    ...
    /**
     * Ritorna il numero medio di posti letto calcolato in base a tutte le abitazioni disponibili
     *
     * @return il numero medio di posti letto calcolato in base a tutte le abitazioni disponibili
     */
